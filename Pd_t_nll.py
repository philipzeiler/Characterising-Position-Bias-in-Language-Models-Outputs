...
# ---------------- parameters ---------------------------
CTX          = 2048
PD_MIN       = -6000               # left bound on x-axis
...
vmax_auto = np.nanmax(mean_mat)    # <-- NEW: dynamic top of colour map
vmax_auto = 3 #I hardcoded it for testing
cmap = sns.color_palette("RdYlGn_r", as_cmap=True)

im = plt.imshow(mean_mat,
                origin="lower",
                extent=[LEFT, RIGHT,         # <-- use real x range
                        0, GRID_Y-1],
                aspect="auto",
                cmap=cmap,
...
plt.tight_layout()
plt.savefig("nll_pd_t_heatmap.png", dpi=300)
plt.show()