...
colors = [cmap(i / max(len(FAMILIES)-1, 1)) for i in range(len(FAMILIES))]
...
for fi, fam in enumerate(FAMILIES):
    ...
    fam_maxL = fam.get("MAX_DOC_LEN", DEFAULT_MAX_DOC_LEN)  # ← NEW per-family override
    models   = fam["models"]
...
# x-axis: model size in log2 scale
...
ax.set_ylabel("Token output position bias in NLL (Δ max–min)")

ax.legend(loc="upper left", frameon=True, handlelength=4, borderaxespad=0.4)
ax.set_title("Output position bias vs model size (per family; log2 x-scale)")

plt.tight_layout()
plt.savefig(
    "D:/Sync/Sync/ETH Stuff/Bachelor Thesis/Code/graphs/opbtok_vs_model_size_by_family.pdf",
    format="pdf",
    bbox_inches="tight",
)
plt.show()
