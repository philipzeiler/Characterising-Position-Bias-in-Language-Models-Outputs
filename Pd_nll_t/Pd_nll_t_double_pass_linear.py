#!/usr/bin/env python3
"""
This is a version of the graph with a linear x-axis. The other file uses a logarithmic scale x-axis.
...
def t_bin_labels(t_arr):
    ...
    idx = np.searchsorted(T_EDGES, t_arr, side="right") - 1    # 0 … 8
...
sns.lineplot(data=df_plot, x="P_d", y="NLL", hue="t_bin", palette="viridis")

plt.xlabel("P_d  (doc-start position in context window)")
plt.ylabel("NLL (nats)")
plt.title(f"NLL vs P_d  ({N_target:,} rows per t-bin, stratified)")
plt.tight_layout()
plt.show()
